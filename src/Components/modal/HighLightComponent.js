...
const HighLightComponent = ({ formik }) => {
  const [items, setItems] = useState([...formik.values.highlights]);
  ...
  const addItem = async () => {
    if (!input) {
      ...
      setInput("");
      await formik.setFieldValue("highlights", [...items, input]);
    }
  };

  const deleteItem = (id) => {
    const updateItems = formik.values.highlights.filter((item, i) => {
      return i !== id;
    });

    formik.setFieldValue("highlights", updateItems);
