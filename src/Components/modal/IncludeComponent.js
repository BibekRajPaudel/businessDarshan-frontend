...
const IncludeComponent = ({ formik }) => {
  const [items, setItems] = useState([...formik.values.included]);
  ...
  const addItem = async () => {
    if (!input) {
      ...
      setInput("");
      await formik.setFieldValue("included", [...items, input]);
    }
  };

  const deleteItem = (id) => {
    const updateItems = formik.values.included.filter((item, i) => {
      return i !== id;
    });

    formik.setFieldValue("included", updateItems);
